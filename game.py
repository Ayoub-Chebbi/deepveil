...
def evidence_board_view(screen, language, found_clues, level):
    ...
    start_y = board_rect.y + 150
    
    clue_keys = [f"Clue{i+1}_Level{level}" for i in range(3)]
    evidence_cards = []
    for i, clue in enumerate([f"clue{i+1}_level{level}" for i in range(3)]):
        if clue in found_clues:
            ...
            title = title_font.render(translations[language][clue_keys[i]], True, (255, 255, 255))
            content = content_font.render(translations[language][clue_keys[i]], True, (200, 200, 200))
            
            title_rect = title.get_rect(center=(card_width // 2, 80))
            content_rect = content.get_rect(center=(card_width // 2, card_height // 2 + 100))
    ...
    while running:
        ...
        title_rect = title_text.get_rect(center=(WIDTH // 2, board_rect.y + 80))
        ...
        draw_button(screen, back_button, (0, 100, 200), (0, 150, 255), translations[language]["Back"], font, pygame.mouse.get_pos())
